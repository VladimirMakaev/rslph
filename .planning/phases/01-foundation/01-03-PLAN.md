# Plan 01-03: Progress File Parser/Writer
...
This plan implements the progress file format that rslph uses to track task execution state. It creates a markdown-based format with all required sections (PROG-01 through PROG-07), a parser using pulldown-cmark, and atomic file writes using the atomicwrites crate.

**Requirements covered:** PROG-01, PROG-02, PROG-03, PROG-04, PROG-05, PROG-06, PROG-07

**Independence:** This plan has no dependencies on 01-01 or 01-02. It can run in parallel with 01-01 (Wave 1).

**Key insights from research:**
- Enable `Options::ENABLE_TASKLISTS` for checkbox parsing
- Use `TaskListMarker(bool)` event for checkbox state
- Atomic writes via atomicwrites crate handles Windows edge cases
- fsync before rename for durability

## Tasks

<task id="1">
<title>Define progress file data structures</title>
<what>
Create the ProgressFile struct and related types that represent all sections of the progress file format.
</what>
<how>
...
<done>
- ProgressFile struct with all 7 sections defined
- TaskPhase, Task, Attempt, IterationEntry types defined
- is_done() checks for RALPH_DONE marker
- total_tasks(), completed_tasks(), next_task() helpers work
- All tests pass
</done>
</task>

<task id="2">
<title>Implement markdown parser</title>
<what>
Create parser that reads markdown progress files and extracts all sections into ProgressFile struct using pulldown-cmark.
</what>
<how>
1. Add to `src/progress.rs`:
   ```rust
   ...
   impl ProgressFile {
           ...
           for event in parser {
               match event {
                   Event::Start(Tag::Heading { level, .. }) => {
                       ...
                       Self::flush_section(&mut pf, &current_h2, &section_text, &current_h3, &mut current_phase_tasks);
                       section_text.clear();
                   ...
                   Event::End(TagEnd::Heading(_)) => {
                       match heading_level {
                           Some(HeadingLevel::H1) => {
                               pf.name = heading_text.trim().trim_start_matches("Progress:").trim().to_string();
                               current_h1 = heading_text.trim().to_string();
                           }
                           ...
                           Some(HeadingLevel::H3) => {
                               ...
                               if current_h2 == "Tasks" && !current_h3.is_empty() && !current_phase_tasks.is_empty() {
                   ...
                   Event::Text(text) => {
                       ...
                       } else if let Some(checked) = current_task_checked.take() {
                       ...
                       if current_h2 == "Iteration Log" && table_row.len() >= 5 {
                           if let Ok(iteration) = table_row[0].parse::<u32>() {
                               pf.iteration_log.push(IterationEntry {
                                   ...
                                   tasks_completed: table_row.get(3).and_then(|s| s.parse().ok()).unwrap_or(0),
       ...
       fn flush_section(pf: &mut ProgressFile, h2: &str, text: &str, h3: &str, phase_tasks: &mut Vec<Task>) {
           let text = text.trim();
           match h2 {
               ...
               "Recent Attempts" => {
                   // Parse attempt blocks
                   Self::parse_attempts(pf, text, h3);
               }
               _ => {}
           }
       }

       fn parse_attempts(pf: &mut ProgressFile, _text: &str, h3: &str) {
           // If h3 is "Iteration N", parse attempt
           if let Some(num_str) = h3.strip_prefix("Iteration ") {
               if let Ok(iteration) = num_str.trim().parse::<u32>() {
                   // For now, create placeholder - full parsing in future iteration
                   if !pf.recent_attempts.iter().any(|a| a.iteration == iteration) {
                       pf.recent_attempts.push(Attempt {
                           iteration,
                           tried: String::new(),
                           result: String::new(),
                           next: None,
...
<done>
- ProgressFile::parse() parses markdown into struct
- Status section extracted correctly
- Analysis section extracted
- Tasks with phases and checkboxes parsed
- Completed/incomplete task state detected
- Testing strategy section extracted
- Iteration log table parsed
- All parsing tests pass
</done>
</task>

<task id="3">
<title>Implement atomic markdown writer</title>
<what>
Create writer that serializes ProgressFile back to markdown and writes atomically using temp file + rename pattern for crash safety.
</what>
<how>
1. Add to `src/progress.rs`:
   ```rust
   ...
   impl ProgressFile {
       ...
       pub fn write(&self, path: &Path) -> Result<(), RslphError> {
       ...
       pub fn load(path: &Path) -> Result<Self, RslphError> {
               ...
               if phase.name == phase_name {
                   for task in &mut phase.tasks {
                       if task.description == task_description && !task.completed {
                           task.completed = true;
                           self.completed_this_iteration.push(task_description.to_string());
                           return true;
                       }
                   }
               }
       ...
       pub fn log_iteration(&mut self, iteration: u32, started: &str, duration: &str, tasks_completed: u32, notes: &str) {
...
2. Add roundtrip and atomic write tests:
   ```rust
   #[cfg(test)]
   mod tests {
       use super::*;
       ...
       fn test_complete_task() {
           let mut pf = ProgressFile {
               tasks: vec![TaskPhase {
                   name: "Phase 1".to_string(),
                   tasks: vec![
                       Task { description: "Task A".to_string(), completed: false },
                       Task { description: "Task B".to_string(), completed: false },
                   ],
               }],
               ..Default::default()
           };
...
3. Add tempfile to dev-dependencies in Cargo.toml:
   ```toml
   [dev-dependencies]
   tempfile = "3"
   ```
...
<done>
- ProgressFile::to_markdown() generates valid markdown
- ProgressFile::write() uses atomic temp file + rename
- ProgressFile::load() reads and parses from disk
- Roundtrip test passes (parse -> to_markdown -> parse)
- complete_task() marks task done and adds to completed_this_iteration
- mark_done() sets RALPH_DONE status
- All tests pass including atomic write test
</done>
</task>

## Verification

- [ ] `cargo test` passes all progress file tests
- [ ] Sample progress file can be parsed and re-serialized
- [ ] Task completion state correctly detected from checkboxes
- [ ] RALPH_DONE marker detection works
- [ ] Atomic write creates temp file then renames
- [ ] Progress file survives roundtrip (parse -> write -> parse)
- [ ] All 7 sections from requirements are supported

## must_haves

Critical requirements that MUST be true for this plan to be considered complete:

- [ ] **PROG-01**: Status section with RALPH_DONE marker detection
- [ ] **PROG-02**: Analysis/research section parsed and written
- [ ] **PROG-03**: Task list with phases and checkboxes (completed state)
- [ ] **PROG-04**: Testing strategy section parsed and written
- [ ] **PROG-05**: Completed This Iteration section tracked
- [ ] **PROG-06**: Recent Attempts section for failure memory
- [ ] **PROG-07**: Iteration log table parsed and written
- [ ] Atomic writes via temp file + rename (crash-safe)
- [ ] Roundtrip preserves data (parse -> to_markdown -> parse)
- [ ] Task checkbox state (checked/unchecked) correctly handled
