...
must_haves:
  truths:
    ...
    - "Footer displays key binding hints and log path (if available)"
  artifacts:
    - path: "src/tui/ui.rs"
...
<objective>
Create the header status bar and progress widgets that display iteration count, task count, model name, project name, and context usage bar with traffic light coloring. Footer displays key hints and log path.

Purpose: Provide the user with at-a-glance status information about the build progress. The 2-line header is always visible and gives immediate feedback on iteration progress, task completion, and context window usage. The footer shows key hints AND the log path so users can access the full conversation log (TUI-03).

Output: Fully functional status bar rendering with proper layout, progress gauge, traffic light colors, and log path display.
</objective>
...
- Two-part footer: key hints on left, log path on right
- Key hints: "j/k:scroll  {/}:iteration  p:pause  Ctrl+C:quit"
- Log path: if app.log_path is Some, display path; otherwise omit
- Style: dim/gray text
...
fn render_footer(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let log_display = app.log_path
        .as_ref()
        .map(|p| format!("Log: {}", p.display()))
        .unwrap_or_default();

    if log_display.is_empty() {
        // Just key hints, centered or left-aligned
        let paragraph = Paragraph::new(key_hints)
...
Layout struct from ratatui is used correctly with Constraint::Length and Constraint::Fill.
  ...
  <done>
Main render function exists with 3-area layout (header, body, footer with log path).
  </done>
</task>
...
4. Footer shows key binding hints on left, log path on right (if set)
...
- Footer displays keyboard shortcut hints AND log path (TUI-03)
