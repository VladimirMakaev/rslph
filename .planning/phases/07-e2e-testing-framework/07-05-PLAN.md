...
must_haves:
  ...
  key_links:
    - from: "tests/e2e/test_rslph_integration.rs"
      to: "src/config.rs"
      via: "RSLPH_CLAUDE_PATH env var overrides claude_path config"
      pattern: "RSLPH_CLAUDE_PATH"
    - from: "tests/e2e/test_rslph_integration.rs"
      to: "rslph binary"
      via: "assert_cmd invokes rslph binary"
...
Note: This plan closes the integration gap identified by the checker. The mechanism is RSLPH_CLAUDE_PATH env var (already supported by rslph config via Env::prefixed("RSLPH_")) or workspace .rslph/config.toml with custom claude_path.
</objective>
...
// Infrastructure tests (from Plan 04)
mod test_basic_loop;
mod test_edge_cases;

// True E2E integration tests (this plan)
...
This module will contain tests that actually invoke the `rslph` binary with fake Claude.
  </action>
  <verify>
...
The key mechanism: Set RSLPH_CLAUDE_PATH env var to point to fake Claude binary. The rslph config loader already supports this via `Env::prefixed("RSLPH_")` (see src/config.rs line 99).
...
<task type="auto">
  <name>Task 3: Add TUI-disabled mode verification</name>
  <files>tests/e2e/test_rslph_integration.rs</files>
  <action>
...
- Integration gap from checker verification is closed
</success_criteria>
