...
must_haves:
  truths:
    ...
    - "Testing phase is visible before test execution"
  artifacts:
...
3. Add `TrialEventKind::Testing` event before test execution in `run_single_trial`:
   - After build completes, before `run_project_tests`, send Testing event via callback
   - This requires adding a second callback or enum for phase events
   - ALTERNATIVE (simpler): Add a `phase_callback: Option<Arc<dyn Fn(&str) + Send + Sync>>` that accepts phase names like "testing"

For simplicity, just wire Building events first. Testing event can be added later.
  </action>
