...
must_haves:
  truths:
    ...
    - "Thinking blocks have collapse state tracked in App (toggle key binding in Plan 06)"
  ...
  key_links:
    ...
    - from: "src/tui/ui.rs"
      to: "app.thinking_collapsed"
      via: "render_conversation call passes collapse state"
      pattern: "thinking_collapsed"
---
...
2. Modify render_item function signature to accept collapse state:
   ```rust
   fn render_item(item: &ConversationItem, is_collapsed: bool) -> Vec<Line<'static>>
   ```

3. Update render_conversation to enumerate items with index:
   ```rust
   for (index, item) in items.iter().enumerate() {
       // Check collapse state for Thinking items
       let is_collapsed = match item {
           ConversationItem::Thinking(_) => thinking_collapsed.get(&index).copied().unwrap_or(false),
           _ => false,
       };
       let lines = render_item(item, is_collapsed);
       all_lines.extend(lines);
   }
   ```

4. Modify render_item for Thinking variant:
   - Use Block with BorderType::Rounded and DarkGray border
   - Title: " Thinking " (with spaces for padding)
   - When collapsed (is_collapsed = true), show: "▶ {N} chars" instead of full content
   - When expanded, show italic gray text with "▼" indicator
   - Add collapse indicator: "▼" for expanded, "▶" for collapsed

5. Modify render_item for ToolUse variant:
...
6. Modify render_item for ToolResult variant:
...
<task type="auto">
  ...
  <files>src/tui/conversation.rs, src/tui/ui.rs</files>
  <action>
...
2. Pass the collapse state to render_item for Thinking items:
   - Track item index (as shown in Task 2)
   - For Thinking items, check if collapsed and render accordingly

3. Update caller in src/tui/ui.rs (render function, around line 50-55):
   Change:
   ```rust
   render_conversation(
       frame,
...
4. Note: src/tui/plan_tui.rs also calls render_conversation but uses its own PlanTuiState which does NOT have thinking_collapsed. For plan_tui.rs, pass an empty HashMap:
   ```rust
   render_conversation(
       frame,
       main_area,
...
`grep -n "thinking_collapsed" src/tui/ui.rs` shows passing app.thinking_collapsed.
  </verify>
  <done>render_conversation accepts collapse state and callers updated</done>
</task>
...
2. App has toggle_thinking_collapse, is_thinking_collapsed, and toggle_all_thinking_collapsed methods
...
8. render_item accepts is_collapsed boolean parameter
9. src/tui/ui.rs passes &app.thinking_collapsed to render_conversation
10. src/tui/plan_tui.rs passes empty HashMap to render_conversation
